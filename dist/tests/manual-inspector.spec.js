...
const inspector_config_1 = require("./inspector.config");
const TARGET_URL = inspector_config_1.INSPECTOR_CONFIG.targetUrl;
// Передаём locatorAttributes из конфига в фикстуру — применяется для всего файла
src_1.test.use({
    smartInspector: {
        enabled: true,
        locatorAttributes: inspector_config_1.INSPECTOR_CONFIG.locatorAttributes,
    },
});
// ─── Основной тест: открываем сайт и ждём ручного взаимодействия ─────────────
(0, src_1.test)('🔍 Manual Inspector — onlytests.io text-generator', async ({ page }) => {
    console.log('\n' + '═'.repeat(62));
    console.log('  🚀 Открываем:', TARGET_URL);
    console.log('  🔑 Атрибуты P1:', inspector_config_1.INSPECTOR_CONFIG.locatorAttributes.join(', '));
    console.log('═'.repeat(62));
    await page.goto(TARGET_URL, { waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle');
    await (0, src_1.expect)(page).toHaveURL(TARGET_URL);
    const inspectorReady = await page.evaluate(() => typeof window.onLocatorGenerated === 'function');
...
(0, src_1.test)('📝 Demo — разведка элементов onlytests.io', async ({ page }) => {
    await page.goto(TARGET_URL, { waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle');
    console.log('\n' + '─'.repeat(62));
    console.log('  🧪 Demo: собираем интерактивные элементы страницы');
    console.log('─'.repeat(62));
    const elements = await page.evaluate((attrs) => {
    ...
    }, inspector_config_1.INSPECTOR_CONFIG.locatorAttributes);
    console.log('\n  📋 Найденные интерактивные элементы:');
    elements.forEach((el, i) => {
        const parts = [`<${el.tag}>`];
        if (el.customAttr && el.attrName)
            parts.push(`[${el.attrName}="${el.customAttr}"]  ← P1`);
        else if (el.id)
            parts.push(`#${el.id}  ← P1 (id)`);
        else if (el.role)
            parts.push(`role="${el.role}"  ← P2`);
        else if (el.text)
            parts.push(`"${el.text}"  ← P3`);
        else if (el.cls)
            parts.push(`.${el.cls}  ← P5`);
